...
# Эксперимент 3: prefix dropout

## Research question

**Может ли prefix dropout при обучении seqrec улучшить обобщение модели, особенно для tail-рекомендаций?**

Гипотеза: если при обучении случайно обрезать SID треков до первого кода (грубого «жанрового» уровня), модель будет вынуждена обобщать по префиксу и лучше предсказывать редкие треки.

## Сетап

Prefix dropout — это аугментация на этапе обучения seqrec. dVAE **не переобучается**. Оба прогона (baseline и dropout) используют одни и те же SID из `prefix/sids.parquet` (supervised).

При обучении с вероятностью p каждый трек в истории пользователя обрезается до первых keep кодов SID, а UNIQUE-токен выбрасывается. При инференсе (eval) дропаут отключен - тестовые данные не модифицируются.

| Параметр | Baseline | Prefix Dropout |
|---|---|---|
| prefix_dropout_prob | 0 | 0.2 |
| prefix_dropout_keep | — | 1 |
| Остальные настройки | идентичны | идентичны |

Конфигурация seqrec:

| Параметр | Значение |
|---|---|
| Dataset | Yambda subset |
| Train users / Test users | 812K / 19K |
| Train interactions | ~79M |
| SID | VarLen dVAE (prefix-supervised), до 6 кодов |
| SeqRec depth | 4 |
| History budget | 128 tokens |
| Beam size | 50 |
| GPU | NVIDIA L4 (16 GB) |

Все прогоны (baseline и dropout keep=1/2/3) обучались с одним конфигом (`device_batch_size: 256`, `ideal_batch_size_tokens: 65536`). Абсолютные значения метрик не сопоставимы с Экспериментом 1 (другой SID, другой конфиг); значимо только относительное сравнение внутри этой таблицы.

## Результаты

| Метрика | Baseline (p=0) | Dropout (p=0.2) | Δ | Δ (%) |
|---|---:|---:|---:|---:|
| Recall@10 | 0.011624 | 0.014359 | +0.002735 | +23.53% |
| Recall@50 | 0.024141 | 0.037921 | +0.013780 | +57.08% |
| NDCG@10 | 0.006720 | 0.007174 | +0.000454 | +6.76% |
| NDCG@50 | 0.009450 | 0.012232 | +0.002782 | +29.44% |
| Coverage@10 | 0.017023 | 0.012535 | −0.004488 | −26.36% |
| Head Recall@10 | 0.015795 | 0.019581 | +0.003786 | +23.97% |
| Tail Recall@10 | 0.000197 | 0.000000 | −0.000197 | −100.00% |
| Long-tail share | 0.067676 | 0.009755 | −0.057921 | −85.59% |

Recall@50 совпадает с Recall@100, потому что `beam_size=50` генерирует ровно 50 кандидатов на пользователя.

## Интерпретация

Prefix dropout **улучшает общие метрики**: Recall@10 растет на +23.5%, Recall@50 на +57.1%, NDCG@10 на +6.8%, NDCG@50 на +29.4%, head Recall@10 на +24.0%. Модель лучше предсказывает популярные треки.

Однако **гипотеза о tail-рекомендациях не подтвердилась**:

- Tail Recall@10 упал до нуля (−100%).
- Long-tail share сократилась с 6.8% до 1.0% (−85.6%).
- Coverage@10 снизилась на −26.4%.

При обрезке SID до одного кода модель видит только грубый «жанровый» уровень, не различая конкретных треков внутри жанра. Это заставляет её сильнее полагаться на популярность, а не на уникальные признаки трека.

## Выводы

Prefix dropout (p=0.2, keep=1) - эффективная аугментация для повышения общего Recall/NDCG, но она ухудшает разнообразие и tail-рекомендации. Метод делает модель более head-biased.

Возможные направления улучшения:

- `keep=2` вместо `keep=1` - оставить больше структуры в обрезанном SID;
- Комбинация с supervised SID (prefix/sids.parquet), где первые коды осмысленны (artist/album);
- Меньшее `p` (0.05–0.1);
