...
Prefix dropout — это аугментация на этапе обучения seqrec. dVAE **не переобучается**. Оба прогона (baseline и dropout) используют одни и те же SID из `original/sids.parquet`.
...
| SID | VarLen dVAE (original), до 6 кодов |
| SeqRec depth | 4 |
| History budget | 128 tokens |
| Beam size | 50 |
| GPU | NVIDIA L4 (16 GB) |

Из-за ограниченной памяти GPU (16 GB вместо A100) batch size был уменьшен: `device_batch_size: 64`, `ideal_batch_size_tokens: 16384`. Для воспроизводимости сравнения baseline был обучен с тем же уменьшенным batch size.
